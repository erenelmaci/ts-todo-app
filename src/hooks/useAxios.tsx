...
const useAxios = () => {
  ...
  const getTodos = async () => {
    const { data } = await axios.get<TodoType[]>(
      "https://63f757b2e8a73b486af5c406.mockapi.io/api/todos"
  ...
  const toggleTodo: ToggleFn = async (item) => {
    ...
    const { data } = await axios.put<TodoType>(
      `https://63f757b2e8a73b486af5c406.mockapi.io/api/todos/${item.id}`,
      updateTodo
    )
    if (data) {
      getTodos()
  ...
  const deleteTodo: DeleteFn = async (id) => {
    try {
      const res = await axios.delete<TodoType>(
        `https://63f757b2e8a73b486af5c406.mockapi.io/api/todos/${id}`
      )
      console.log(res)
      getTodos()
    } catch (error) {
